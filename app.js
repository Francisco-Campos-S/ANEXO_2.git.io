// Variables globales
let datos = null;
/** Evita llamadas a Gemini durante la inicialización; se activa tras pintar (solo interacción real del usuario). */
let geminiIAHabilitadaTrasCarga = false;
...
function inicializarApp() {
    ...
    configurarIAResultadosIndicadores();
    ...
    /* La IA solo debe dispararse por acciones humanas (isTrusted), no por restore/dispatch al cargar */
    requestAnimationFrame(() => {
        requestAnimationFrame(() => {
            geminiIAHabilitadaTrasCarga = true;
        });
    });
}
...
/** Clave API Google AI (Gemini). Defínala en index.html como window.GEMINI_API_KEY */
...
/** Cache de planes “modelo + versión API” (sesión). */
let geminiPlanGeneracionCache = null;

function puntuarNombreModeloGemini(name) {
    const n = String(name).toLowerCase();
    if (n.includes('embed') || n.includes('aqa') || n.includes('imagen') || n.includes('tts')) return 900;
    if (n.includes('flash-lite')) return 0;
    if (n.includes('2.5') && n.includes('flash')) return 1;
    if (n.includes('2.0') && n.includes('flash')) return 2;
    if (n.includes('1.5') && n.includes('flash')) return 3;
    if (n.includes('pro')) return 10;
    return 5;
}

async function obtenerListaModelosApi(apiVer, key, signal) {
    const out = [];
    let pageToken = '';
    for (let p = 0; p < 15; p++) {
        let url = `https://generativelanguage.googleapis.com/${apiVer}/models?key=${encodeURIComponent(key)}`;
...
/**
 * Descubre modelos con generateContent y construye intentos (v1 y/o v1beta por modelo).
 */
async function construirPlanIntentosGemini(key, signal) {
    if (geminiPlanGeneracionCache) return geminiPlanGeneracionCache;

    const porNombre = new Map();

    for (const apiList of ['v1', 'v1beta']) {
        let lista;
        try {
            lista = await obtenerListaModelosApi(apiList, key, signal);
        } catch {
            lista = [];
        }
        for (const m of lista) {
            ...
            if (!short) continue;
            if (!porNombre.has(short)) porNombre.set(short, new Set());
            porNombre.get(short).add(apiList);
        }
    }
    ...
    for (const [short, versiones] of ordenados) {
        for (const apiVer of ['v1', 'v1beta']) {
            if (versiones.has(apiVer)) {
                plan.push({ modelo: short, apiVer });
            }
        }
    }

    if (plan.length > 0) {
        geminiPlanGeneracionCache = plan;
        return plan;
    }

    const fallback = [];
    for (const modelo of GEMINI_MODELOS_INTENTO) {
        for (const apiVer of ['v1', 'v1beta']) {
            fallback.push({ modelo, apiVer });
        }
    }
    return fallback;
}

function resolverTipoAnexoDesdeNodo(nodo) {
    if (nodo && nodo.closest && nodo.closest('#seccionMatematica')) return 'mat';
    return 'esp';
}

function parseErrorRespuestaGemini(textoCuerpo, statusHttp) {
    if (!textoCuerpo || typeof textoCuerpo !== 'string') return statusHttp || 'Error desconocido';
    try {
        const j = JSON.parse(textoCuerpo);
        const err = j.error;
        if (err && err.message) {
            const codigo = err.code != null ? `[${err.code}] ` : '';
            const estado = err.status ? `${err.status}: ` : '';
            return `${codigo}${estado}${err.message}`;
        }
    } catch (_) {
        /* no JSON */
    }
    if (textoCuerpo.length > 400) return textoCuerpo.slice(0, 400) + '…';
    return textoCuerpo || String(statusHttp);
}

function origenNoPermiteFetchApi() {
    if (typeof location === 'undefined') return false;
    const p = location.protocol;
    return p === 'file:' || p === 'chrome-extension:' || p === 'moz-extension:';
}

/** Llama a Gemini y devuelve texto plano; prueba varios modelos y devuelve el mensaje de error real de Google. */
async function generarContenidoGemini(prompt, signal) {
    const key = obtenerClaveGemini();
    if (!key) throw new Error('Sin clave API (window.GEMINI_API_KEY).');

    if (origenNoPermiteFetchApi()) {
        throw new Error(
            'Abra el sitio con un servidor HTTP (p. ej. Live Server en VS Code, o python -m http.server), no como archivo file://. El navegador bloquea llamadas a la API desde archivos locales.'
        );
    }

    let ultimoError = 'No hubo respuesta de ningún modelo.';

    let plan;
    try {
        plan = await construirPlanIntentosGemini(key, signal);
    } catch (e) {
        if (e.name === 'AbortError') throw e;
        plan = GEMINI_MODELOS_INTENTO.flatMap((modelo) => [
            { modelo, apiVer: 'v1' },
            { modelo, apiVer: 'v1beta' }
        ]);
    }

    const intentos = plan.slice(0, GEMINI_MAX_INTENTOS_GENERACION);

    for (const { modelo, apiVer } of intentos) {
        if (signal?.aborted) {
        ...
        try {
            ...
            ultimoError =
                netErr && netErr.message
                    ? `Red/CORS: ${netErr.message}. Use https:// o un servidor local.`
                    : 'Fallo de red o CORS.';
            continue;
        }
        ...
        if (!res.ok) {
            ultimoError = `${parseErrorRespuestaGemini(raw, res.status)} (${apiVer}/${modelo})`;
            if (res.status === 401 || res.status === 403) {
                geminiPlanGeneracionCache = null;
                throw new Error(parseErrorRespuestaGemini(raw, res.status));
            }
            if (res.status === 429) {
                continue;
            }
            const es404 = res.status === 404 || /not found/i.test(ultimoError);
            const esModeloNoDisponible =
                es404 || /is not found|not supported|invalid argument|INVALID_ARGUMENT/i.test(ultimoError);
            if (esModeloNoDisponible) continue;
            throw new Error(parseErrorRespuestaGemini(raw, res.status));
        }
        ...
        try {
            ...
            ultimoError = 'Respuesta no válida del servidor.';
            continue;
        }

        const cand = data.candidates?.[0];
        if (!cand) {
            const fb = data.promptFeedback?.blockReason || data.promptFeedback?.blockReasonMessage;
            ultimoError = fb ? `Bloqueado: ${fb}` : parseErrorRespuestaGemini(raw, res.status);
            continue;
        }

        const motivo = cand.finishReason;
        if (motivo === 'SAFETY' || motivo === 'BLOCKLIST') {
            ultimoError = `Generación bloqueada (${motivo}).`;
            continue;
        }
        ...
        ultimoError = motivo ? `Sin texto (finishReason: ${motivo}).` : 'El modelo devolvió texto vacío.';
    }

    let finalMsg = ultimoError;
    if (/RESOURCE_EXHAUSTED|quota|429|free_tier|limit:\s*0/i.test(finalMsg)) {
        finalMsg +=
            ' — Si dice “limit: 0” o agotó la cuota gratuita, habilite facturación en Google AI Studio / Cloud o espere (el propio error indica “Please retry in …s”).';
    }
    throw new Error(finalMsg);
}

/** Tema/habilidad para prompts de resultados por código de apoyo (Anexo 2 y tabla apoyos Anexo 10). */
function construirTemaDesdeCodigoApoyo(asignatura, codigo, descripcion) {
    const partes = [];
    if (asignatura) partes.push(asignatura);
    if (codigo && descripcion) partes.push(`${codigo}: ${descripcion}`);
    else if (descripcion) partes.push(descripcion);
    else if (codigo) partes.push(String(codigo));
    return partes.join(' — ') || descripcion || codigo || 'el apoyo educativo registrado';
}

/** Nivel marcado en la fila de indicadores (Inicial / Intermedio / Avanzado). */
function obtenerNivelIndicadorDesdeFila(fila) {
    const cbs = fila.querySelectorAll('.checkbox-nivel');
    const etiquetas = ['Inicial', 'Intermedio', 'Avanzado'];
    for (let i = 0; i < cbs.length; i++) {
        if (cbs[i].checked) return etiquetas[i];
    }
    return null;
}

/** Textarea de resultados en fila de tabla indicadores (Anexo 10). */
function obtenerTextareaResultadosIndicador(fila) {
    const tds = fila.querySelectorAll('td');
    const tdRes = tds[4];
    return tdRes ? tdRes.querySelector('textarea') : null;
}

function configurarIAResultadosIndicadores() {
    const tbody = document.getElementById('tablaIndicadores_mat');
    if (!tbody || tbody.dataset.iaIndicadoresAttached) return;
    tbody.dataset.iaIndicadoresAttached = '1';

    const encolar = (fila) => {
        if (!fila) return;
        if (fila._debounceIAInd) clearTimeout(fila._debounceIAInd);
        fila._debounceIAInd = setTimeout(() => {
            solicitarResultadoIAIndicador(fila).catch((err) => {
    ...
    tbody.addEventListener(
        ...
        (e) => {
            ...
            if (!e.target.classList.contains('input-indicador')) return;
            encolar(e.target.closest('tr'));
        },
        true
    );
    ...
    tbody.addEventListener('change', (e) => {
        if (!e.isTrusted) return;
        if (e.target.classList.contains('checkbox-nivel')) {
            encolar(e.target.closest('tr'));
        }
    });
}
...
async function solicitarResultadoIAIndicador(fila) {
    ...
    const taInd = fila.querySelector('.input-indicador');
    const temaBase = taInd?.value?.trim() || '';
    if (!temaBase) return;
    ...
    const tema = asignMat ? `${asignMat} — ${temaBase}` : temaBase;

    let instruccionNivel;
    if (nivel) {
        instruccionNivel = `El nivel a considerar es: ${nivel}. Debe quedar explícito o claro en el informe.`;
    } else {
        instruccionNivel =
            'No hay casilla de nivel marcada: infiere de forma breve si el desempeño se ubica en nivel Inicial, Intermedio o Avanzado según el indicador.';
    }

    const prompt = `Redacta un informe breve (máximo 30 palabras) como si fueras el docente a cargo, describiendo el desempeño de un estudiante (sin mencionar su nombre) respecto al tema o habilidad: ${tema}, indicando si su nivel es: Inicial, Intermedio o Avanzado. Ajusta el tono y contenido del informe según el nivel.

${instruccionNivel}

Responde solo con el informe, en español, sin saludo ni título.`;

    if (taRes._aiAbort) taRes._aiAbort.abort();
    taRes._aiAbort = new AbortController();
    const { signal } = taRes._aiAbort;

    const phPrev = taRes.placeholder;
    taRes.placeholder = 'Generando sugerencia…';
    taRes.classList.add('ia-cargando');
    taRes.disabled = true;

    try {
        const texto = await generarContenidoGemini(prompt, signal);
        if (texto) {
            taRes.value = texto;
            taRes.dispatchEvent(new Event('input', { bubbles: true }));
        }
    } catch (err) {
        if (err.name === 'AbortError') return;
        console.error('Gemini indicador:', err);
        const detalle =
            err && err.message ? err.message : 'Error desconocido al llamar a Gemini.';
        const corto = detalle.length > 280 ? `${detalle.slice(0, 277)}…` : detalle;
        mostrarNotificacion('No se generó el texto', corto, 'error');
    } finally {
        taRes.placeholder = phPrev;
        taRes.classList.remove('ia-cargando');
        taRes.disabled = false;
    }
}
...
async function solicitarResultadoIAApoyo(select) {
    ...
    if (textareaRes._aiAbort) textareaRes._aiAbort.abort();
    textareaRes._aiAbort = new AbortController();
    const { signal } = textareaRes._aiAbort;

    const tipo = resolverTipoAnexoDesdeNodo(select);
    const asign = document.getElementById(`asignatura_${tipo}`)?.value?.trim() || '';

    const codigo = selectedOption.value;
    const descripcion =
        selectedOption.dataset.descripcion ||
        row.querySelector('.descripcion-apoyo-anexo10')?.textContent?.trim() ||
        '';

    const tema = construirTemaDesdeCodigoApoyo(asign, codigo, descripcion);

    const prompt = `Redacta un informe breve (máximo 40 palabras) como si fueras el docente a cargo, describiendo el desempeño de un estudiante (sin mencionar su nombre) respecto al tema o habilidad: ${tema}.

Responde solo con el informe, en español, sin saludo ni título.`;

    const phPrev = textareaRes.placeholder;
    textareaRes.placeholder = 'Generando sugerencia…';
    textareaRes.classList.add('ia-cargando');
    textareaRes.disabled = true;

    try {
        const texto = await generarContenidoGemini(prompt, signal);
        if (texto) {
            textareaRes.value = texto;
            textareaRes.dispatchEvent(new Event('input', { bubbles: true }));
        }
    } catch (err) {
        if (err.name === 'AbortError') return;
        ...
        const detalle =
            err && err.message
                ? err.message
                : 'Error desconocido al llamar a Gemini.';
        const corto = detalle.length > 280 ? `${detalle.slice(0, 277)}…` : detalle;
        mostrarNotificacion('No se generó el texto', corto, 'error');
    } finally {
        textareaRes.placeholder = phPrev;
        textareaRes.classList.remove('ia-cargando');
        textareaRes.disabled = false;
    }
}
...
function configurarEventosCodigosAnexo10() {
    ...
    selects.forEach(select => {
        select.addEventListener('change', (e) => actualizarDescripcionApoyoAnexo10(e.target, e));
    });
}

// Actualizar descripción del apoyo (Anexo 2 y Anexo 10, misma tabla)
function actualizarDescripcionApoyoAnexo10(select, evento) {
    const selectedOption = select.options[select.selectedIndex];
    const row = select.closest('tr');
    ...
    if (selectedOption.value && selectedOption.dataset.descripcion) {
        ...
        const dispararIA = geminiIAHabilitadaTrasCarga && evento && evento.isTrusted === true;
        if (dispararIA) {
            solicitarResultadoIAApoyo(select).catch((err) => {
                if (err && err.name !== 'AbortError') console.error(err);
            });
        }
    } else {
